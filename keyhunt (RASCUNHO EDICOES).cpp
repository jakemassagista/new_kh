...
// Adicionar após as outras definições globais
struct address_checkpoint_t {
    char range_start[65];
    char range_end[65];
    char current_key[65];
...
const char *version = "3.69 Puzzle BTC - Single Core Persistent";
...
void save_address_checkpoint(Int* current_key, uint64_t count);

bool load_address_checkpoint(Int* start_key);

void calcualteindex(int i,Int *key);

void format_total(Int *value, char *buffer);
...
const char *str_limits_prefixs[8] = {"Kkeys","Mkeys","Bkeys","Tkeys","Quakeys","Quikeys","Sexkeys","Setkeys"};
const char *str_limits[8] = {"1000","1000000","1000000000","1000000000000","1000000000000000","1000000000000000000","1000000000000000000000","1000000000000000000000000"};
Int int_limits[8];
...
int main(int argc, char **argv)	{
	...
	while ((c = getopt(argc, argv, "deh6MqRSB:b:c:C:E:f:I:k:l:m:N:n:p:r:s:t:v:G:8:z:P:")) != -1) {
		switch(c) {
			...
			case 'P':
				...
				printf("[+] Usando salvamento no arquivo de checkpoint: %s\n", optarg);
				break;
			case 'z':
	...
	if(FLAGMODE != MODE_BSGS )	{
		...
		printf("[+] N = %" PRIu64 " (%p)\n", (uint64_t)N_SEQUENTIAL_MAX, (void*)N_SEQUENTIAL_MAX);
			...
			if(MPZAUX.IsZero()) {
				...
				if(pretotal.IsLower(&int_limits[0]))	{
					...
					if(FLAGMATRIX)	{
						...
						sprintf(buffer,"[+] Total %s keys | elapsed: %s seconds | ~%s %s/s (%s keys/s)\n", str_total,str_seconds,str_divpretotal, str_limits_prefixs[salir ? i : i-1],str_pretotal);
					}
					else	{
						if(THREADOUTPUT == 0)	{
							...
							sprintf(buffer,"\r[+] Total %s keys | elapsed: %s seconds | ~%s %s/s (%s keys/s)\r",total_buffer,str_seconds,str_divpretotal, str_limits_prefixs[salir ? i : i-1],str_pretotal);
						}
						else	{
							//sprintf(buffer,"\r[+] Total %s keys in %s seconds: ~%s %s (%s keys/s)\r",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal);
							sprintf(buffer,"\r[+] Total %s keys | elapsed: %s seconds | ~%s %s/s (%s keys/s)\r",total_buffer,str_seconds,str_divpretotal, str_limits_prefixs[salir ? i : i-1],str_pretotal);
...
void *thread_process(void *vargp)	{
    ...
    //uint64_t count = 0;
	//uint64_t checkpoint_interval = 16777216; // Salvar a cada 16 milhões de chaves na troca de bloco N_SEQUENTIAL_MAX
	uint64_t last_checkpoint = 0;
	uint64_t current_total_keys = 0;
	...
	// Definir chave inicial do checkpoint se existir na global n_range_start
	if (thread_number == 0 && FLAGCHECKPOINT) {
		pthread_mutex_lock(&write_random);
		load_address_checkpoint(&n_range_start);
		//printf("Chave inicial: %s\n", key_mpz.GetBase16());
		pthread_mutex_unlock(&write_random);		
	}

	// Definir chave inicial da global
			...
			do {
				...
				*/
				...
				for(j = 0; j < CPU_GRP_SIZE/4;j++){
					...
					//total_keys.fetch_add(4);
					current_total_keys+=4;
					//count+=4;
					
					//if (thread_number == 0 && count % 10000000 == 0) { // Salvar checkpoint a cada 10 milhões com a thread 0
					//	save_address_checkpoint(&key_mpz, count);
						//printf("\n[+] Criando checkpoint em %" PRIu64 " chaves verificadas, chave salva: %s\n", count, key_mpz.GetBase16());
					//}

					// Salvar checkpoint periodicamente na thread 0
					if (thread_number == 0 && FLAGCHECKPOINT) {
						//current_total_keys = total_keys.load();
						if ((current_total_keys-last_checkpoint) >= 10000000) {
							//printf("\n[+] Criando checkpoint em %" PRIu64 " chaves verificadas\n", count);
							//pthread_mutex_lock(&write_random);
							last_checkpoint = current_total_keys;							
							save_address_checkpoint(&key_mpz, current_total_keys);
							//pthread_mutex_unlock(&write_random);
				...
				*/
				...
				pp.y.ModSub(&_2Gn.y);
				startP = pp;
			}while(current_total_keys < N_SEQUENTIAL_MAX && continue_flag);
		}
	} while(continue_flag);
	ends[thread_number] = 0;
	return NULL;
}
...
void menu() {
	...
	printf("-P file     Usar checkpoint no modo ADDRESS (salvar/carregar progresso)\n");
...
void save_address_checkpoint(Int* current_key, uint64_t count) {
    if (!FLAGCHECKPOINT || !checkpoint_file) return;
    
    FILE* fd = fopen(checkpoint_file, "wb");
    if (!fd) {
        fprintf(stderr, "\n[E] Não foi possível criar arquivo de checkpoint %s: %s\n", 
                checkpoint_file, strerror(errno));
        return;
    }

    struct address_checkpoint_t cp;
    memset(&cp, 0, sizeof(struct address_checkpoint_t));
    
    // Salva os valores em hexadecimal
    char* temp = current_key->GetBase16();
    strncpy(cp.current_key, temp, 64);
    free(temp);
    
    //temp = range_start.GetBase16();
    strncpy(cp.range_start, range_start, 64);
    //free(temp);
    
    
    strncpy(cp.range_end, range_end, 64);
    
    
    cp.keys_checked = total_keys_amount + count;
    cp.timestamp = time(NULL);

    // Calcula o hash SHA256 da estrutura
    uint8_t hash[32];
    sha256((uint8_t*)&cp, sizeof(cp) - sizeof(time_t), hash);
    
    // Escreve a estrutura e o hash
    size_t written_struct = fwrite(&cp, sizeof(struct address_checkpoint_t), 1, fd);
    size_t written_hash = fwrite(hash, 32, 1, fd);
    
    if (written_struct != 1 || written_hash != 1) {
        fprintf(stderr, "\n[E] Erro ao salvar checkpoint: %s\n", strerror(errno));
    } //else {
        //printf("\n[+] Checkpoint salvo em 0x%s (%" PRIu64 " chaves verificadas)\n", 
               //cp.current_key, cp.keys_checked);
    //}
...
bool load_address_checkpoint(Int* start_key) {
    if (!FLAGCHECKPOINT || !checkpoint_file) return false;
    
    FILE* fd = fopen(checkpoint_file, "rb");
    if (!fd) {
        printf("[+] Nenhum checkpoint encontrado, iniciando do começo (0x%s)\n", start_key->GetBase16());
        return false;
    }

    struct address_checkpoint_t cp;
    
    // Lê a estrutura do checkpoint
    size_t read_struct = fread(&cp, sizeof(struct address_checkpoint_t), 1, fd);
    if (read_struct != 1) {
        fprintf(stderr, "[E] Erro ao ler estrutura do checkpoint: %s\n", strerror(errno));
        fclose(fd);
        return false;
    }

    // Verifica se os campos estão preenchidos corretamente
    if (strlen(cp.current_key) == 0 || strlen(cp.range_start) == 0 || strlen(cp.range_end) == 0) {
        fprintf(stderr, "[E] Checkpoint corrompido: campos vazios\n");
        fclose(fd);
        return false;
    }

    // Converte e verifica os valores
    Int temp_start, temp_current, temp_end;
    
    // Removemos as verificações de retorno já que SetBase16 é void
    temp_start.SetBase16(cp.range_start);
    temp_end.SetBase16(cp.range_end);
    temp_current.SetBase16(cp.current_key);
	
    // Verifica se o range corresponde ao atual
    if (!temp_start.IsEqual(&n_range_start) || !temp_end.IsEqual(&n_range_end)) {
        fprintf(stderr, "[E] Range do checkpoint não corresponde ao atual\n");
        fclose(fd);
        return false;
    }

    // Verifica se a chave atual está dentro do range válido
    if (temp_current.IsGreater(&temp_end) || temp_current.IsLower(&temp_start)) {
        fprintf(stderr, "[E] Chave do checkpoint fora do range válido\n");
        fclose(fd);
        return false;
    }

    // Se chegou até aqui, o checkpoint é válido
	
    start_key->Set(&temp_current);

    //Int total_keys_calc;
    //total_keys_calc.Sub(&temp_current, &temp_start); // total_keys_calc = current_key - range_start
    //*count = total_keys_calc.GetInt64(); // Atribui o valor calculado a *count

	Int total_keys_chekeds;
	total_keys_chekeds.SetInt64(cp.keys_checked);
	char total_progress[256];
	//char total_calc[256];

	format_total(&total_keys_chekeds, total_progress);
	//format_total(&total_keys_calc, total_calc);
    
    printf("[+] Checkpoint carregado com sucesso:\n");
    printf("    - Retomando da chave: 0x%s (%" PRIu64 ")\n", cp.current_key, start_key->GetInt64());
    //printf("    - Progresso salvo: [%s] \n", total_keys_chekeds.GetBase10());
	printf("    - Progresso salvo: [%s] \n", total_progress);
    
    
    fclose(fd);
	total_keys_amount = cp.keys_checked;
	n_range_start.Set(start_key);
...
void format_total(Int *value, char *buffer) {
    Int div_value;
    char *str_value, *str_div_value;
    int i = 0, salir = 0;
    
    if(value->IsLower(&int_limits[0])) {
        str_value = value->GetBase10();
        sprintf(buffer, "%s", str_value);
        free(str_value);
    } else {
        while(i < 6 && !salir) {
            if(value->IsLower(&int_limits[i+1])) {
                salir = 1;
            } else {
                i++;
            }
        ...
        div_value.Set(value);
        div_value.Div(&int_limits[salir ? i : i-1]);
        str_div_value = div_value.GetBase10();
        sprintf(buffer, "~%s %s", str_div_value, str_limits_prefixs[salir ? i : i-1]);
        free(str_div_value);
    }
}
